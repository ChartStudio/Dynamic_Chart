...
class DataFlowConfig {
  ...
  constructor(datas: DataFlow[]) {
    this.xAxisDataList = datas.map((value: DataFlow) : number[] => value.data.map((value: GraphData) : number => value.x));
    this.yAxisDataList = datas.map((value: DataFlow) : number[] => value.data.map((value: GraphData) : number => value.y));
    this.graphDataList = datas.map((value: DataFlow) : GraphData[] => value.data)
    this.lineConfigList = datas.map((value: DataFlow) : LineStyleConfig => new LineStyleConfig(value.style));
  }
