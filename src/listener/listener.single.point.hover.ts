...
export const singlePointMouseMoveListener = (x: number, y: number, helper: SinglePointHoverHelper, event: MouseEvent) => {
  ...
  if (helper.isPopUp === false && helper.isLocationValid(findLoaction.index, findLoaction.lineIndex)) {
    ...
    helper.graphTaskController.singlePointPopUp(helper.savedLoaction.lineIndex, helper.savedLoaction.index)
  } else if (helper.isPopUp === true && !helper.isLocationValid(findLoaction.index, findLoaction.lineIndex)) {
    helper.changeStatusPopUp()
    helper.graphTaskController.singlePointPopDown(helper.savedLoaction.lineIndex, helper.savedLoaction.index)
    helper.savedLoaction = findLoaction
...
export const singlePointMouseLeaveListener = (event: MouseEvent, helper: SinglePointHoverHelper) => {
  event.stopPropagation()
...
export default SinglePointHoverHelper;